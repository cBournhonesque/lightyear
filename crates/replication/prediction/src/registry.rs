...
impl PredictionRegistry {
    ...
    fn record_confirmed_and_maybe_check<C: SyncComponent>(
        ...
        // A confirmed value arriving for a component this entity has never predicted (no
        // PredictionHistory<C>) and does not currently have (no live C) — e.g. the server
        // inserted a marker like `Dead` mid-game that the client cannot predict — would otherwise
        // have no rollback membership (#1692). Seed the prediction history with what the client
        // actually predicted — "absent" — so the normal mismatch -> rollback -> restore pipeline
        // can apply the server-side insert.
