...
//!
//! No peer owns the deterministic world: the application supplies its founding state, and the
//! archive supplies the inputs and later player activations.
//!
...
impl Default for JoinCatchUpConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            // One hour at 60 Hz; exceeding this bound makes the session unjoinable.
...
///
/// Complete from the first gameplay tick, including player activations needed by later newcomers.
#[derive(Resource, Debug)]
pub struct InputHistory<S> {
    records: Vec<RecordedInput<S>>,
    joins: Vec<SessionJoin>,
    /// Earliest recorded tick across independently arriving player streams.
    start_tick: Option<Tick>,
    /// Latest recorded tick, used with `start_tick` to bound the retained span.
    end_tick: Option<Tick>,
    /// Latest recorded tick per player; prevents duplicates across retransmissions.
    recorded_until: HashMap<u64, Tick>,
    /// Set when the session outgrew [`JoinCatchUpConfig::max_history_ticks`].
    truncated: bool,
    /// Founding gameplay tick; input delivery may begin later.
...
impl<S> InputHistory<S> {
    ...
    fn record(&mut self, hash: u64, tick: Tick, sequence: S, max_history_ticks: u32) -> bool {
        ...
        // Players arrive independently, so bound the entire retained span.
...
fn record_input_history<S: ActionStateSequence>(
    ...
    for (pre_spawned, buffer, local) in &players {
        ...
        // Never archive remote prediction: held-forward values cannot be corrected afterward.
        // Advance each stream contiguously; early inputs can arrive several frames late.
        ...
        while next <= confirmed_until {
            // A one-tick sequence must carry a resolved value, not SameAsPrecedent.
...
/// Feed replay inputs before the input pipeline reads them; the live ring cannot hold a session.
...
fn request_catch_up<S: ActionStateSequence>(
    ...
    *history = InputHistory::default();
    ...
    tracing::info!(peer = ?trigger.remote_peer, ?donor, "asked the admission coordinator for its inputs");
}
...
fn serve_catch_up_request<S: ActionStateSequence>(
    ...
    for (receiver, inputs, end) in &mut links {
        ...
        for request in receiver.receive() {
            ...
            for chunk in history.records.chunks(records_per_message) {
            ...
            let checksum = target.and_then(|target_tick| {
                ...
                let (checksum, entries) =
                    crate::checksum::compute_history_checksum_with_count(&mut world, target_tick);
                (entries > 0).then_some(checksum)
            });
            end.send::<P2PChannel>(CatchUpEnd {
                id: request.id,
...
fn receive_catch_up<S: ActionStateSequence>(
    ...
    let mut initial_peers = session.started_peers();
    initial_peers.retain(|peer| !history.joins.iter().any(|join| join.peer == *peer));
    apply_catch_up::<S>(
        &mut commands,
        timeline.tick(),
        tick_duration.0,
...
/// Merge the archive with retained live inputs and request a replay.
#[allow(clippy::too_many_arguments)]
fn apply_catch_up<S: ActionStateSequence>(
    ...
    initial_peers: SmallVec<[PeerId; 4]>,
    ...
    // The application can rebuild only the founding world, not a missing historical prefix.
    ...
    // Rebuild and run forward; a newcomer has no historical world to roll back.
...
/// Unconsumed snapshots, streamed one tick at a time rather than overflowing the input rings.
#[derive(Resource, Debug)]
...
pub struct P2PCatchUpReplay {
    ...
    /// Founding peers, excluding players activated by historical `P2PJoined` events.
    ///
    /// Derived from the accepted incumbent roster, not today's sorted lobby roster.
    pub initial_peers: SmallVec<[PeerId; 4]>,
}
...
/// Rebuild once, then advance bounded batches with a private fixed clock.
/// Networking retains its live clock between batches.
fn run_catch_up_replay<S: ActionStateSequence>(world: &mut World) {
    ...
    if initialize {
        ...
        *world.resource_mut::<Time>() = fixed_time.as_generic();
        let initial_peers =
            core::mem::take(&mut world.resource_mut::<CatchUpReplay>().initial_peers);
    ...
    for _ in 0..(batch_end - batch_start).max(0) {
        ...
        for join in joins.iter().filter(|join| join.tick == next_tick) {
            let link = world
                .resource::<lightyear_connection::network_topology::NetworkingMetadata>()
                .peer_map
                .get(&join.peer)
                .copied();
        ...
        *world.resource_mut::<Time>() = world.resource::<Time<Fixed>>().as_generic();
        ...
        if verify && world.resource::<LocalTimeline>().tick() == target_tick {
            ...
            let (checksum, entries) = crate::checksum::compute_history_checksum_with_count(
                &mut checksum_world,
                target_tick,
            );
            let checksum = (entries > 0).then_some(checksum);
            let mut replay = world.resource_mut::<CatchUpReplay>();
