...
use lightyear::websocket::client::{WebSocketClientIo, WebSocketScheme};
...
impl LobbyPeer {
    fn new(slot: u8, base_port: u16, policy: LobbyIdPolicy) -> Self {
        ...
        // Plain `ws`, no TLS: this test is about the dial, not about certificates.
...
fn explicit_retry_reconnects_websocket_members_in_both_directions() {
    ...
    let original_dialer = (0..2)
        .find(|slot| {
            stepper.peers[*slot]
                .app
                .world()
                .get::<WebSocketClientIo>(live_links[*slot])
                .is_some()
        })
    ...
    for (cycle, retrying) in [original_dialer, original_acceptor].into_iter().enumerate() {
        // Close the actual session, not the endpoint: the other lobby must learn about the
        // transport closure itself, and both listening sockets remain available for recovery.
