...
fn test_input_custom_rebroadcast() {
    ...
    // Add rooms only for clients 0 and 1 (but NOT the entity, so it stays visible to all clients)
    let room = stepper
        .server_app
        .world_mut()
    ...
    stepper
        .server_app
        .world_mut()
        .entity_mut(client_of0)
        .insert(Rooms::single(room));
    stepper
        .server_app
        .world_mut()
        .entity_mut(client_of1)
        .insert(Rooms::single(room));
    stepper.frame_step_server_first(1);
