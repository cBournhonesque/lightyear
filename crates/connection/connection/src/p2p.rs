...
pub enum P2P {
    ...
    /// [`P2PSessionPhase`]. Candidates appear in the cached roster for synchronization and input
    /// routing, but not in its started-player set.
    Candidate,
...
///
/// Candidate connectivity is tracked separately: startup waits for every declared candidate,
/// while input routing can reach only connected candidates.
