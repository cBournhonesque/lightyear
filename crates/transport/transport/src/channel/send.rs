...
///
/// `ChannelSend` owns messages buffered by the application until the transport either sends or
/// discards them according to the channel's delivery policy. It also applies the channel's send
/// cadence, fragments oversized messages, schedules reliable retries, and reports per-frame
/// acknowledgement events.
///
/// Sending is transactional: eligible messages are first exposed as lightweight candidates for
/// global prioritization and packet packing. A candidate is committed only after its packet enters
/// the link's send queue, and the channel finalizes its queues once the complete send flush ends.
/// The private state enum contains only the queueing and acknowledgement state which differs
/// between reliable and unreliable channels.
#[derive(Debug)]
...
impl ChannelSend {
    ...
    /// Appends snapshots of messages which are eligible for this send flush.
    ///
    /// Candidates participate in global priority ordering and packet packing, but collecting them
    /// does not remove messages from the channel or mark reliable messages as sent. The candidate
    /// only clones the message's [`Bytes`] handle, so an unsuccessful staging attempt leaves the
    /// channel unchanged.
    ...
    /// Commits one candidate after the packet containing it enters the link's send queue.
    ///
    /// Unreliable messages are marked for removal at the end of the flush. Reliable messages stay
    /// pending, but record `sent_at` so retransmission waits for the configured resend delay.
    ...
    /// Finalizes channel state after all packets for the current flush have been staged.
    ///
    /// This removes committed unreliable candidates and applies
    /// [`ChannelSettings::retry_unsent_messages`] when bandwidth prevented eligible unreliable
    /// messages from being sent. Reliable messages remain pending until acknowledged.
...
impl UnreliableSendState {
    ...
    fn commit_send(&mut self, key: SendMessageKey) {
        match key {
            SendMessageKey::UnreliableSingle(index) => {
                let Some(pending) = self.singles.get_mut(index) else {
                    debug_assert!(false, "invalid unreliable single send candidate");
                    return;
                };
                pending.committed = true;
            }
            SendMessageKey::UnreliableFragment(index) => {
                let Some(pending) = self.fragments.get_mut(index) else {
                    debug_assert!(false, "invalid unreliable fragment send candidate");
                    return;
                };
                ...
                pending.committed = true;
                if mark_started {
                    for pending in &mut self.fragments {
            ...
            _ => debug_assert!(false, "invalid unreliable send candidate"),
        }
    }
