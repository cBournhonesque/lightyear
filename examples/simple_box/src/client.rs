...
pub(crate) fn create_plugin(
    ...
    let rivet_lobby_data = crate::rivet::client::find_lobby().expect("rivet::find_lobby");
    dbg!("rivet_lobby_data: {:?}", rivet_lobby_data);
    let player_token = &rivet_lobby_data["player"]["token"];
