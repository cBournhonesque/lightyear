...
The example can also run as a deterministic input-only game with no server. Peers discover each
other through a lobby instead of a preconfigured roster:
...
Every peer creates the same physics world, predicts all players, and rolls back both entity and
Avian simulation state when remote input is late.
