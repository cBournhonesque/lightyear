...
pub struct ServerSettings {
    ...
    /// If true, we will run prediction for other clients as well
    /// (otherwise we will simply run interpolation)
    predict_all: bool,
            ...
            let server_plugin_group = IoTaskPool::get()
                .scope(|s| {
                    s.spawn(Compat::new(async {
                        ServerPluginGroup::new(settings.transport, settings.predict_all, shared)
                            .await
                    }));
                })
                .pop()
