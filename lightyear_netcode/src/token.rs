//! Connect-token types used by the netcode handshake.
//!
//! A [`ConnectToken`] is usually generated by a trusted backend or by
//! [`Server::token`](crate::server::Server::token), serialized to [`CONNECT_TOKEN_BYTES`], and then
//! passed to a netcode client. The public token contains the server addresses and encrypted private
//! data that only a server with the shared private key can read.
...
/// List of public or internal server socket addresses embedded in a connect token.
///
/// Netcode tokens support up to 32 addresses. Clients use the public address list when attempting
/// to connect. Servers validate against the private/internal address list stored in the encrypted
/// token data.
...
impl AddressList {
    const IPV4: u8 = 1;
    const IPV6: u8 = 2;
    /// Creates an address list from a socket address source.
    ///
    /// At most 32 addresses are stored; additional addresses are ignored.
    ...
    /// Iterates over stored addresses and their free-list indices.
...
/// Encrypted private section of a [`ConnectToken`].
///
/// This data is readable only by servers that know the shared private key. It contains the client
/// ID, timeout, internal server address list, session keys, and application user data.
pub struct ConnectTokenPrivate {
    /// Client ID assigned by the token issuer.
    pub client_id: u64,
    /// Timeout, in seconds, used by the client/server session.
    pub timeout_seconds: i32,
    /// Server addresses the server uses to validate the token destination.
    pub server_addresses: AddressList,
    /// Symmetric key used for client-to-server payloads.
    pub client_to_server_key: Key,
    /// Symmetric key used for server-to-client payloads.
    pub server_to_client_key: Key,
    /// Application-defined user data copied into server connection callbacks.
...
impl ConnectTokenPrivate {
    ...
    /// Encrypts this private token data into the fixed-size private token buffer.
    pub fn encrypt(
        &self,
        protocol_id: u64,
        expire_timestamp: u64,
    ...
    /// Decrypts private token data from the fixed-size private token buffer.
    pub fn decrypt(
        encrypted: &mut [u8],
        protocol_id: u64,
        expire_timestamp: u64,
...
/// Challenge token exchanged during the netcode handshake.
///
/// Servers use this token to prove that the client can respond to a server challenge without
/// exposing the connect token private data in plaintext.
pub struct ChallengeToken {
    /// Client ID assigned by the connect token.
    pub client_id: u64,
    /// Application-defined user data copied from the connect token.
    pub user_data: [u8; USER_DATA_BYTES],
}

impl ChallengeToken {
    /// Serialized and encrypted challenge-token size in bytes.
    pub const SIZE: usize = 300;
    /// Encrypts this challenge token for the given packet sequence.
    ...
    /// Decrypts a challenge token for the given packet sequence.
    pub fn decrypt(
