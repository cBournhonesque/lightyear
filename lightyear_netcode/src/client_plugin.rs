//! Bevy integration for netcode clients.
//!
//! [`crate::client_plugin::NetcodeClientPlugin`] runs netcode in
//! [`ConnectionSystems`](lightyear_connection::ConnectionSystems), between raw link IO and
//! `lightyear_transport`. Incoming link packets are decrypted and validated before transport
//! systems see them; outgoing transport payloads are wrapped as netcode payload packets before link
//! systems flush them.
...
/// Plugin that drives [`NetcodeClient`] entities.
///
/// The plugin installs the base client connection plugin, orders link/connection/transport system
/// sets, and observes [`Connect`] and [`Disconnect`] triggers.
pub struct NetcodeClientPlugin;

/// Component that makes an entity a netcode-authenticated client.
///
/// The [`Link`] and [`lightyear_connection::client::Client`] components are required
/// automatically. Construct this with [`NetcodeClient::new`], then trigger [`Connect`] to start the
/// handshake.
#[derive(Component)]
...
/// Configuration for a [`NetcodeClient`] component.
///
/// This is the Bevy-facing configuration wrapper. It is converted into the lower-level
/// [`ClientConfig`] used by the protocol state machine.
pub struct NetcodeConfig {
    /// Number of redundant disconnect packets sent when the client disconnects.
    pub num_disconnect_packets: usize,
    /// Interval, in seconds, for netcode keepalive/control packets.
...
impl NetcodeClient {
    /// Creates a netcode client component from an authentication source and config.
    ///
    /// `Authentication::Token` is the production path. `Authentication::Manual` is useful for tests
    /// where the client can build a token locally.
    ...
    /// Returns the Lightyear peer ID represented by this netcode client.
    ///
    /// Before the protocol finishes connecting, the underlying netcode client ID is still the
    /// placeholder value used by the low-level client.
