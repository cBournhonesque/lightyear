//! Bevy integration for netcode servers.
//!
//! [`crate::server_plugin::NetcodeServerPlugin`] runs netcode in
//! [`ConnectionSystems`](lightyear_connection::ConnectionSystems), between raw link IO and
//! `lightyear_transport`. The plugin validates connection tokens, creates server-side client link
//! entities, inserts [`TokenUserData`], and wraps outgoing transport payloads as netcode packets.
...
/// Plugin that drives [`NetcodeServer`] entities and server-side netcode client links.
...
pub struct NetcodeServer {
    /// Low-level netcode server state machine and callback context.
    pub(crate) inner: crate::server::Server<NetcodeServerContext>,
}

// TODO: should be part of the NetcodeServer component
/// Configuration for a [`NetcodeServer`] component.
///
/// This is the Bevy-facing configuration wrapper. It is converted into the lower-level
/// [`ServerConfig`] used by the protocol state machine.
#[derive(Debug, Clone)]
pub struct NetcodeConfig {
    /// Number of redundant disconnect packets sent when disconnecting a client.
    ...
    /// Application protocol ID. Clients and servers must use the same value.
    pub protocol_id: u64,
    /// Private key used to decrypt connect tokens.
    pub private_key: Key,
}
...
impl NetcodeConfig {
    /// Sets the application protocol ID.
    pub fn with_protocol_id(mut self, protocol_id: u64) -> Self {
        self.protocol_id = protocol_id;
        self
    }
    /// Sets the private key used to validate connect tokens.
    pub fn with_key(mut self, key: Key) -> Self {
        self.private_key = key;
        self
    }

    /// Sets the timeout, in seconds, embedded in tokens generated by this server.
...
impl NetcodeServer {
    /// Creates a netcode server component from Bevy-facing configuration.
    ///
    /// Connection and disconnection callbacks are wired to an internal context so the plugin can
    /// insert and despawn server-side client entities after the low-level server processes packets.
