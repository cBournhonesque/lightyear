...
pub struct NetcodeConfig {
    ...
    /// Set the duration (in seconds) after which the server disconnects a paused client if they don't hear from them.
    /// A negative value means no timeout.
    ///
    /// A connection is considered paused if the client sends a packet requesting to pause the connection.
    /// This is usually done on the web, where tabs that are in the background are throttled. We do not want to
    /// throttle those connection so instead we "pause" them, meaning that we stop sending packets to the client
    /// and we increase the timeout.
    pub paused_client_timeout_secs: i32,
    pub protocol_id: u64,
    pub private_key: Key,
}
...
impl Default for NetcodeConfig {
    fn default() -> Self {
        Self {
            ...
            // 5 minutes
            paused_client_timeout_secs: 300,
            protocol_id: 0,
