use std::{
    io::BufReader, net::SocketAddr, sync::Arc
};
...
use tokio::{
    ...
    sync::{mpsc::{self, error::TryRecvError, UnboundedReceiver, UnboundedSender}, Mutex},
};
...
impl WebSocketServerSocket {
    ...
    pub async fn handle_client(
        ...
        to_client_channels: Arc<std::sync::Mutex<HashMap<SocketAddr, UnboundedSender<WebSocketMessage>>>>,
    ) {
        let mut ws = fut.await.unwrap();
        ws.set_writev(false); // TODO understand this
        let ws = Arc::new(Mutex::new(FragmentCollector::new(ws)));

        let (to_client_sender, mut to_client_receiver) = mpsc::unbounded_channel::<WebSocketMessage>();
                ...
                match recv.lock().await.read_frame().await {
                    Ok(frame) => match frame.opcode {
                        OpCode::Close => {
                            ...
                            from_client_sender.send((WebSocketMessage::Close(None, None), client_addr)).unwrap();
                            to_client_channels.lock().unwrap().remove(&client_addr);
                            break;
                        },
                        OpCode::Binary => {
                            trace!("received packet from client!: {:?}", &frame.payload);
                            from_client_sender.send((WebSocketMessage::Binary(frame.payload.into()), client_addr)).unwrap();
                        },
                        _ => {}
                    },
                    Err(e) => {
                ...
                if let Some(msg) = to_client_receiver.recv().await {
                    match msg {
                        WebSocketMessage::Close(code, reason) => {
                            ...
                            send.lock().await.write_frame(Frame::close(code.unwrap_or(1000), reason.unwrap_or("".to_string()).as_bytes())).await;
                            break;
                        },
                        WebSocketMessage::Binary(data) => {
                            trace!("sending packet to client!: {:?}", &data);
                            send.lock().await.write_frame(Frame::binary(Payload::Owned(data))).await;
                        }
                    }
                }
            }
    ...
    fn get_tls_acceptor(&self) -> Option<TlsAcceptor> {
        if let Some(config) = &self.tls_config {
            let server_config = ServerConfig::builder()
                .with_no_client_auth()
                .with_single_cert(
                    certs(&mut BufReader::new(&*config.certs)).map(|e| e.unwrap()).collect(),
                    rsa_private_keys(&mut BufReader::new(&*config.keys)).map(|e| e.unwrap().into()).next().unwrap(),
                )
                .unwrap();
...
struct WebSocketServerSocketSender {
    server_addr: SocketAddr,
    to_client_senders: Arc<std::sync::Mutex<HashMap<SocketAddr, UnboundedSender<WebSocketMessage>>>>,
}
...
impl PacketSender for WebSocketServerSocketSender {
    fn send(&mut self, payload: &[u8], address: &SocketAddr) -> std::io::Result<()> {
        if let Some(to_client_sender) = self.to_client_senders.lock().unwrap().get(address) {
            to_client_sender.send(WebSocketMessage::Binary(payload.into())).map_err(|e| {
...
impl PacketReceiver for WebSocketServerSocketReceiver {
    fn recv(&mut self) -> std::io::Result<Option<(&mut [u8], SocketAddr)>> {
        match self.from_client_receiver.try_recv() {
            Ok((data, addr)) => {
                match data {
                    WebSocketMessage::Binary(mut buf) => Ok(Some((&mut buf.to_owned(), addr))),
                    _ => Ok(None),
                }
            }
            Err(e) => {
