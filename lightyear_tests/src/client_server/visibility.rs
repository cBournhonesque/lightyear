...
fn test_spawn_gain_visibility() {
    ...
    // gain visibility
    stepper.client_apps[0].world_mut().get_mut::<NetworkVisibility>(client_entity).unwrap().gain_visibility(stepper.client_entities[0]);
    stepper.frame_step(1);
...
fn test_despawn_lose_visibility() {
    ...
    stepper.client_apps[0].world_mut().get_mut::<NetworkVisibility>(client_entity).unwrap().lose_visibility(stepper.client_entities[0]);
    stepper.frame_step(1);
    assert!(stepper.client_of(0).get::<MessageManager>().unwrap().entity_mapper.get_local(client_entity).is_none());
}
...
fn test_despawn_with_visibility() {
    ...
    let server_entity_0 = stepper.server_app.world_mut().spawn((
    ...
    let server_entity_1 = stepper.server_app.world_mut().spawn((
        Replicate::to_clients(NetworkTarget::All),
        visibility_1,
        ReplicationGroup::new_id(1),
    )).id();
    ...
    let client_entity_0 = stepper.client(0).get::<MessageManager>().unwrap().entity_mapper.get_local(server_entity_0).unwrap();
    let client_entity_1 = stepper.client(1).get::<MessageManager>().unwrap().entity_mapper.get_local(server_entity_1).unwrap();
    ...
    stepper.client_mut(1).get_mut::<MessageManager>().unwrap().entity_mapper.insert(server_entity_0, client_entity_1);

    stepper.server_app.world_mut().despawn(server_entity_0);
    stepper.frame_step(2);

    // client entity 0 has been despawned
    assert!(stepper.client_apps[0].world().get_entity(client_entity_0).is_err());

    // client entity 1 has not been despawned (because connection 1 did not have visibility on server_entity_0)
    assert!(stepper.client_apps[1].world().get_entity(client_entity_1).is_ok());
}
