...
/// Test that components are synced from Confirmed to Predicted and that PredictionHistory is
    ...
    fn test_predicted_sync_batch() {
        ...
        stepper.client_app().add_observer(
            |trigger: Trigger<OnAdd, CompSimple>,
             query: Query<(), With<CompOnce>>| {
        ...
        stepper.client_app().add_observer(
            |trigger: Trigger<OnAdd, CompOnce>,
             query: Query<(), With<CompSimple>>| {
                assert!(query.get(trigger.target()).is_ok());
            },
        );
    ...
    fn test_update_history() {
        ...
        fn update_comp_full(
            mut full: Single<&mut CompFull, With<Predicted>>,
        ){
            full.0 += 1.0;
        }
        stepper.client_app()
