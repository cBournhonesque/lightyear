...
#[test]
    ...
    fn test_prespawn_local_despawn_match() {
        ...
        stepper.client_mut(0).get_mut::<InputTimeline>().unwrap().0.context.config.max_error_margin = 0.5;
        stepper.client_mut(0).get_mut::<Link>().unwrap().recv.conditioner = Some(
        ...
        let confirmed = stepper
            ...
            .get::<Predicted>(client_prespawn)
            .unwrap();
        assert_eq!(stepper.client_app().world().resource::<PredictionMetrics>().rollbacks, 0);
    }