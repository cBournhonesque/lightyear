...
pub struct ReliableSender {
    ...
    messages_to_send: VecDeque<(Option<MessageId>, Message)>,
...
impl ReliableSender {
    ...
    fn collect_messages_to_send(&mut self) {
        ...
        let resend_delay =
            Duration::from_millis((self.rtt_resend_factor * self.current_rtt_millis) as u64);
        ...
        for (message_id, message) in self.unacked_messages.iter_mut() {
            ...
            if should_send {
                self.messages_to_send
                    .push_back((Some(*message_id), message.message.clone()));
...
mod tests {
    ...
    fn test_reliable_sender_internals() {
        ...
        let message = Message::new(Bytes::from("hello"));
