- Pausing:
  - we want to pause reliable channels getting retried
  - we want to pause waiting for the acks to send updates
  - in replication send `apply_replication`, the client should not appear in the list of clients to send
  - in send_message as well?
  - need to re-sync?
