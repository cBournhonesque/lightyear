...
use bevy_enhanced_input::prelude::{ActionBinding, InputAction, UntypedAction};
use core::any::TypeId;
...
impl InputRegistry {
    ...
    pub(crate) fn bind<'r: 'a, 'a>(
        ...
        let metadata = self.input_action_metadata.get(&kind)
            .ok_or_else(|| InputRegistryError::NoMetadataFound(kind))?;
        
        let bind_fn: BindFn<'a> = unsafe { core::mem::transmute(metadata.bind_fn) };
        Ok(bind_fn(bindings, action_map))
    }
}
...
type BindFn<'a> = fn(
    &'a mut Vec<ActionBinding>,
    &'a mut TypeIdMap<UntypedAction>
) -> &'a mut ActionBinding;
...
impl InputActionMetadata {
    fn new<A: InputAction>() -> Self {
        ...
        let bind_fn: for<'a> fn(&'a mut Vec<ActionBinding>, &'a mut TypeIdMap<UntypedAction>) -> &'a mut ActionBinding  = 
            bevy_enhanced_input::input_context::actions::get_or_create_binding_untyped::<A>;
        Self { bind_fn: unsafe { core::mem::transmute(bind_fn)} }
    }
}
...
impl InputRegistryExt for &mut App {
    fn register_input_action<A: InputAction>(self) -> Self {
        ...
        registry.add::<A>();
        trace!("Registered input action: {:?}. registry kinds: {:?}", core::any::type_name::<A>(), registry.kind_map);
        self
    }
}
