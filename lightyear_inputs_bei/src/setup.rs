...
impl InputRegistryPlugin {
    ...
    fn serialize_action<A: InputAction>(message: &Action<A>, writer: &mut Writer) -> std::result::Result<(), SerializationError> {
        Ok(())
    }
    fn deserialize_action<A: InputAction>(reader: &mut lightyear_serde::reader::Reader) -> std::result::Result<Action<A>, SerializationError> {
        Ok(Action::<A>::default())
    }
}
...
impl InputRegistryExt for &mut App {
    fn register_input_action<A: InputAction>(self) -> Self {
        ...
        self.register_component_custom_serde::<Action<A>>(
            SerializeFns::<Action<A>> {
