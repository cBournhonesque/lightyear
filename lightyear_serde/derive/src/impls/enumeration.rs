...
fn get_ser_method(enum_: &DataEnum, bits_needed: u8) -> TokenStream {
    ...
    for (index, variant) in enum_.variants.iter().enumerate() {
        ...
        let base = match &variant.fields {
            Fields::Unit => {
                quote! {
                    Self::#variant_name => {
                        let index = lightyear_serde::UnsignedInteger::<#bits_needed>::new(#variant_index);
                        index.ser(writer);
                    }
                }
            }
    ...
    quote! {
         fn ser(&self, writer: &mut dyn lightyear_serde::BitWrite) {
            match self {
                #ser
            }
         }
...
fn get_de_method(enum_: &DataEnum, bits_needed: u8) -> TokenStream {
    ...
    quote! {
        fn de(reader: &mut lightyear_serde::BitReader) -> std::result::Result<Self, lightyear_serde::SerdeErr> {
            let index: lightyear_serde::UnsignedInteger<#bits_needed> = Serde::de(reader)?;
            let index_u16: u16 = index.get() as u16;
            Ok(match index_u16 {
                #de
                _ => return std::result::Result::Err(lightyear_serde::SerdeErr{})
            })
        }
    }
}
