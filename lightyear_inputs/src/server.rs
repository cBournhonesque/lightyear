...
fn update_action_state<S: ActionStateSequence>(
    ...
    mut action_state_query: Query<(Entity, &mut S::State, &mut InputBuffer<S::Snapshot>, Has<S::Marker>)>,
) {
    ...
    for (entity, mut action_state, mut input_buffer, is_local) in action_state_query.iter_mut() {
        ...
        if let Some(snapshot) = input_buffer.get(tick) {
            S::from_snapshot(action_state.as_mut(), snapshot, &context, is_local);
            trace!(
                ?tick,
                ?entity,
